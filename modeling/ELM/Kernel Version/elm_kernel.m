...
if isa(TrainingData, 'double')~=1, TrainingData = double(TrainingData); end
T=reshape(TrainingLabels,1,numel(TrainingLabels));
P=TrainingData;

%%%%%%%%%%% Load testing dataset
if isa(TestData, 'double')~=1, TestData = double(TestData); end
TV.T=reshape(TestLabels,1,numel(TestLabels));
TV.P=TestData;                                  %   Release raw testing data array
...
elseif strcmp(kernel_type,'lin_kernel') || strcmp(kernel_type,'lin')
    if nargin<4,
        omega = Xtrain*Xtrain';
    else
        omega = Xtrain*Xt';
